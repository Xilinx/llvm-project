...
public:
  ...
  PdllRewritePattern(
      PDLPatternModule pdllModule)
      : RewritePattern(MatchAnyOpTypeTag(), /*benefit=*/1, pdllModule.getModule().getContext()),
  ...
  FailureOr<State> tryMatch(PatternRewriter &rewriter, pdl::PatternOp pattern,
    ...
    for (Operation &constraint :
      ...
      if (auto operation = dyn_cast<pdl::OperationOp>(constraint)) {
        ...
        for(auto it : llvm::zip(operation.getAttributeValueNames(), operation.getAttributeValues())) {
          auto name = cast<StringAttr>(std::get<0>(it)).getValue();
          auto val = state.get( std::get<1>(it)).cast<Attribute>();
          if(target->getAttr(name) !=       val) {
                        return rewriter.notifyMatchFailure(target, Twine("attr doesn't match: ") + name);
          }
        }
        ...
        state.add(result.getResult(),state.get(result.getParent()).cast<Operation*>()->getResult(result.getIndex()));
        ...
        auto fn = pdllModule.getConstraintFunctions().at(applyNativeConstraint.getName());
        SmallVector<PDLValue> args;
        for (auto arg : applyNativeConstraint->getOperands()) {
          args.push_back(state.get(arg));
        }
        if (failed(fn(rewriter, args)) /*!= applyNativeConstraint.getIsNegated()*/)
          return rewriter.notifyMatchFailure(target, Twine("Native constraint failed: ") + applyNativeConstraint.getName());
  ...
  LogicalResult applyRewrite(Operation *op, PatternRewriter &rewriter,
    ...
    for (Operation &child :
      ...
      if (auto applyNativeRewrite =
        ...
        auto fn = pdllModule.getRewriteFunctions().at(applyNativeRewrite.getName());
        SmallVector<PDLValue> args;
...
*/
  ...
  std::unique_ptr<PdllRewritePattern> pattern =
      RewritePattern::create<PdllRewritePattern>(
          std::move(pdlPatterns));
