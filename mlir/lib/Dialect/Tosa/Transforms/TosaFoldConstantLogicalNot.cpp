...
struct TosaFoldConstantLogicalNot : public OpRewritePattern<LogicalNotOp> {

  using OpRewritePattern::OpRewritePattern;

  LogicalResult matchAndRewrite(LogicalNotOp recip,
                                PatternRewriter &rewriter) const override {
    auto inputTensor = recip.getInput1();

    // Check that we can apply folding
    auto preCondCheck =
        notifyIfNotConstantIntegerTosaTensor(inputTensor, recip, rewriter);
    ...
    auto computeLogicalNot = [](const APInt &val, IntegerType) {
      return APInt(1, !val.getBoolValue());
    };

    // Create a new tensor with the updated values
    auto newTensor = applyElementWise<APInt, APInt, IntegerType>(
        inputValues, computeLogicalNot,
        cast<IntegerType>(inputValues.getElementType()));
...
void mlir::tosa::populateTosaFoldConstantLogicalNotPatterns(
