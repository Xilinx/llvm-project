//===- MlirExploreMain.h - MLIR Exploration Driver main ---------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// Main entry function for mlir-explore for when built as standalone binary.
//
//===----------------------------------------------------------------------===//
...
/// Configuration options for the mlir-explore tool.
/// This is intended to help building tools like mlir-explore by collecting the
/// supported options.
...
/// with `MlirExploreMainConfig::registerCLOptions(DialectRegistry &);` and
/// creating a config using `auto config =
/// MlirExploreMainConfig::createFromCLOptions();`.
class MlirExploreMainConfig {
public:
  /// Register the options as global LLVM command line options.
  static void registerCLOptions(DialectRegistry &dialectRegistry);

  /// Create a new config with the default set from the CL options.
  static MlirExploreMainConfig createFromCLOptions();
  ...
  /// general.
  MlirExploreMainConfig &allowUnregisteredDialects(bool allow) {
  ...
  MlirExploreMainConfig &setDebugConfig(tracing::DebugConfig config) {
  ...
  MlirExploreMainConfig &dumpPassPipeline(bool dump) {
  ...
  MlirExploreMainConfig &emitBytecode(bool emit) {
  ...
  MlirExploreMainConfig &setIrdlFile(StringRef file) {
    irdlFileFlag = file;
    return *this;
  }
  ...
  MlirExploreMainConfig &setEmitBytecodeVersion(int64_t version) {
  ...
  /// present).
  MlirExploreMainConfig &runReproducer(bool enableReproducer) {
  ...
  MlirExploreMainConfig &showDialects(bool show) {
  ...
  MlirExploreMainConfig &splitInputFile(bool split = true) {
  ...
  MlirExploreMainConfig &useExplicitModule(bool useExplicitModule) {
  ...
  MlirExploreMainConfig &verifyDiagnostics(bool verify) {
  ...
  MlirExploreMainConfig &verifyPasses(bool verify) {
  ...
  /// Set whether to run the verifier after each transformation pass.
  MlirExploreMainConfig &verifyRoundtrip(bool verify) {
...
/// Perform the core processing behind `mlir-explore`.
...
LogicalResult MlirExploreMain(llvm::raw_ostream &outputStream,
                              std::unique_ptr<llvm::MemoryBuffer> buffer,
                              DialectRegistry &registry,
                              const MlirExploreMainConfig &config);

/// Implementation for tools like `mlir-explore`.
/// - toolName is used for the header displayed by `--help`.
/// - registry should contain all the dialects that can be parsed in the source.
LogicalResult MlirExploreMain(int argc, char **argv, llvm::StringRef toolName,
                              DialectRegistry &registry);

/// Implementation for tools like `mlir-explore`.
/// This function can be used with registrationAndParseCLIOptions so that
/// CLI options can be accessed before running MlirExploreMain.
/// - inputFilename is the name of the input mlir file.
/// - outputFilename is the name of the output file.
/// - registry should contain all the dialects that can be parsed in the source.
LogicalResult MlirExploreMain(int argc, char **argv,
                              llvm::StringRef inputFilename,
                              llvm::StringRef outputFilename,
                              DialectRegistry &registry);

/// Helper wrapper to return the result of MlirExploreMain directly from main.
///
/// Example:
///
///     int main(int argc, char **argv) {
///       // ...
///       return mlir::asMainReturnCode(mlir::MlirExploreMain(
///           argc, argv, /* ... */);
///     }
///
