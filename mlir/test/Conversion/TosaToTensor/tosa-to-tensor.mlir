...
// CHECK-LABEL:  func.func @test_reshape_samerank_unsigned
// CHECK-SAME:   ([[PARAM_0_:%.+]]: tensor<3x2xui8>) -> tensor<2x3xui8> {
// CHECK-DAG:       [[VAR_0_:%.+]] = builtin.unrealized_conversion_cast [[PARAM_0_]] : tensor<3x2xui8> to tensor<3x2xi8>
// CHECK:           [[VAR_collapsed_:%.+]] = tensor.collapse_shape [[VAR_0_]] {{.}}[0, 1]{{.}} : tensor<3x2xi8> into tensor<6xi8>
// CHECK:           [[VAR_expanded_:%.+]] = tensor.expand_shape [[VAR_collapsed_]] {{.}}[0, 1]{{.}} output_shape [2, 3] : tensor<6xi8> into tensor<2x3xi8>
// CHECK:           [[VAR_2_:%.+]] = builtin.unrealized_conversion_cast [[VAR_expanded_]] : tensor<2x3xi8> to tensor<2x3xui8>
// CHECK:           return [[VAR_2_]] : tensor<2x3xui8>
  %cst = tosa.const_shape { value = dense<[2, 3]> : tensor<2xindex> } : () -> !tosa.shape<2>
  %0 = "tosa.reshape"(%arg0, %cst) : (tensor<3x2xui8>, !tosa.shape<2>) -> tensor<2x3xui8>
...
func.func @test_reshape_samerank_dyn(%arg0: tensor<?x2xf32>) -> tensor<2x?xf32> {
  ...
  %0 = "tosa.reshape"(%arg0, %cst): (tensor<?x2xf32>, !tosa.shape<2>) -> tensor<2x?xf32>
