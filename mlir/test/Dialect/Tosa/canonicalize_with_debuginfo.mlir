...
func.func @clamp_twice_is_single_clamp(%arg0: tensor<4xi8>) -> tensor<4xi8> {
  // CHECK: tosa.clamp %arg0 {max_val = 2 : i8, min_val = -2 : i8} {{.*}} loc(#[[FUSED:.*]])
  // CHECK-DAG: #[[A:.*]] = loc("Clamp_A")
  // CHECK-DAG: #[[B:.*]] = loc("Clamp_B")
  // CHECK:     #[[FUSED]] = loc(fused[#[[B]], #[[A]]])
  %0 = tosa.clamp %arg0 {max_val = 4 : i8, min_val = -2 : i8} :  (tensor<4xi8>) -> tensor<4xi8> loc(#loc0)
  %1 = tosa.clamp %0 {max_val = 2 : i8, min_val = -4 : i8} :  (tensor<4xi8>) -> tensor<4xi8> loc(#loc1)
